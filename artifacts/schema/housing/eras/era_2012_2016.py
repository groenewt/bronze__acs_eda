...
class Housing1Year_2012_2016(HousingSurvey):
    """
    1-year housing survey for ACS data years 2012-2016.

    Key characteristics:
    - Uses BDSP, RMSP, VALP (standardized codes)
    - Has BUS (Business_On_Property)
    - Has ADJHSG, ADJINC adjustment factors
    - Has modern appliances: BATH, REFR, SINK, STOV
    - Weights: lowercase wgtp1-80 (2012), uppercase WGTP1-80 (2013+)
    ...
    def _h1y_core_mapping(self) -> Dict[str, str]:
        return {
            ...
            'ST': 'State_Code',
            'ADJHSG': 'Housing_Adjustment_Factor',  # New in 2012
            'ADJINC': 'Income_Adjustment_Factor',   # New in 2012
    ...
    def _h1y_unit_mapping(self) -> Dict[str, str]:
        return {
            ...
            'BATH': 'Bathtub_or_Shower',  # New in 2012
            'BDSP': 'Number_of_Bedrooms',  # Standardized (not BDS)
            'BLD': 'Building_Type',
            'BUS': 'Business_On_Property',  # Still has BUS
            'REFR': 'Refrigerator',  # New in 2012
            'RMSP': 'Number_of_Rooms',  # Standardized (not RMS)
            'RWAT': 'Hot_and_Cold_Running_Water',
            'RWATPR': 'Running_Water',
            'SINK': 'Sink_with_Faucet',  # New in 2012
            'STOV': 'Stove_or_Range',  # New in 2012
            'TEL': 'Telephone_Service',
            'TEN': 'Tenure',
            'VACS': 'Vacancy_Status',
            'VALP': 'Property_Value',  # Standardized (not VAL)
    ...
    def _h1y_flag_mapping(self) -> Dict[str, str]:
        ...
        f1 = [
            'FACRP:Lot_Acreage', 'FAGSP:Agricultural_Sales', 'FBATHP:Bathtub',
            'FBDSP:Bedrooms', 'FBLDP:Building_Type', 'FBUSP:Business_On_Property',
            'FCONP:Condo_Fee', 'FELEP:Electricity', 'FFINCP:Family_Income',
            'FFSP:Food_Stamp', 'FFULP:Fuel', 'FGASP:Gas', 'FGRNTP:Gross_Rent',
        ]
        f2 = [
            'FHFLP:House_Heating_Fuel', 'FHINCP:Household_Income', 'FINSP:Insurance',
            'FKITP:Kitchen', 'FMHP:Mobile_Home', 'FMRGIP:First_Mortgage_Insurance',
            'FMRGP:First_Mortgage_Payment', 'FMRGTP:First_Mortgage_Taxes',
            'FMRGXP:First_Mortgage_Status', 'FMVP:Moved_When', 'FPLMP:Plumbing',
            'FPLMPRP:Plumbing_Project', 'FREFRP:Refrigerator', 'FRMSP:Rooms',
        ]
        f3 = [
            'FRNTMP:Meals_Included_Rent', 'FRNTP:Rent_Amount', 'FRWATP:Running_Water',
            'FRWATPRP:Running_Water_Project', 'FSINKP:Sink',
            ...
            'FSTOVP:Stove', 'FTAXP:Property_Taxes', 'FTELP:Telephone',
            'FTENP:Tenure', 'FVACSP:Vacancy_Status', 'FVALP:Property_Value',
            'FVEHP:Vehicles', 'FYBLP:Year_Built',
        ]
    ...
    def _h1y_weight_mapping(self) -> Dict[str, str]:
        # Handle both lowercase (2012) and uppercase (2013+) weights
        # We'll map both to the same standardized names
        weights = {}
        for i in range(1, 81):
            weights[f'wgtp{i}'] = f'Weight_Replicate_{i}'  # lowercase (2012)
            weights[f'WGTP{i}'] = f'Weight_Replicate_{i}'  # uppercase (2013+)
        return weights

    def get_column_availability(self, year: int) -> Dict[str, bool]:
        """Return which columns are available for the 2012-2016 era."""
